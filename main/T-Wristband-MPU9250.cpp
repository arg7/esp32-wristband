...
*/
    ...
    timeClient.setUpdateInterval(600000); // one hour
...
void RTC_Show()
{
    if (targetTime < millis()) {
        ...
        mm = datetime.minute;
        ss = datetime.second;
        //Serial.printf("yy: %d mn: %d dd: %d hh:%d mm:%d ss:%d\n", yy, mn, dd, hh, mm, ss);
        targetTime = millis() + 1000;
        if (ss == 0 || initial) {
            initial = 0;
            tft.setTextColor(TFT_GREEN, TFT_BLACK);
            tft.setCursor (8, 60);
            char buf[16];
            snprintf( buf, sizeof(buf), "%04d-%02d-%02d", yy, mn, dd); 
            tft.print(buf); // This uses the standard ADAFruit small font
        }
...
*/
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (timeClient.update()) {
        rtc.setDateTime(timeClient.getYear(), timeClient.getMonth(), timeClient.getDay(), timeClient.getHours(), timeClient.getMinutes(), timeClient.getSeconds());
    ...
    case 2:
        ...
        delay(3000);
        esp_sleep_enable_ext1_wakeup(GPIO_SEL_21, ESP_EXT1_WAKEUP_ANY_HIGH);
        //pinMode(39, GPIO_MODE_INPUT);
        //esp_sleep_enable_ext1_wakeup(GPIO_SEL_21+GPIO_SEL_39, ESP_EXT1_WAKEUP_ANY_HIGH);
